...
class KaspiEnrichmentParser
{
    ...
    public function parse(string $html, ?string $url = null): array
    {
        ...
        if (preg_match('/captcha|капча|verify you are human|подтвердите.{0,30}робот/iu', $dom->textContent)) {
            throw new \RuntimeException('captcha_detected');
        }
        // JSON is read from the original HTML; no offers/price/availability fields are extracted.
        $jsonDom = new DOMDocument;
        $previous = libxml_use_internal_errors(true);
        ...
        $meta = ['og_image' => $this->meta($xpath, 'og:image', 'property')];
        $title = $this->firstValue([data_get($item, 'card.title'), ...$this->jsonValues($jsonLd, ['name']), $this->meta($xpath, 'og:title', 'property'), $this->nodeText($xpath->query('//h1')->item(0))]);
        if (trim((string) $title) === '') {
            throw new \RuntimeException('parser_title_missing');
        }
        $rawImages = $this->collectImageCandidates($xpath, $jsonLd, $item, '', $meta);
        [$images] = $this->cleanImages($rawImages);
        $images = array_values(array_filter($images, [KaspiSingleProductPolicy::class, 'imageUrl']));
        if ($images === []) {
    ...
    private function normalizeText(string $value): string
    {
        ...
        $value = str_replace(["\xc2\xa0", '�', 'пїЅ'], [' ', '', ''], $value);
    ...
    private function jsonLd(DOMXPath $xpath, string $url): array
    {
        ...
        $visit = function (array $entry) use (&$visit, &$items, $url): void {
            ...
            $items[] = array_intersect_key($entry, array_flip(['name', 'description', 'image', 'additionalProperty']));
        };
