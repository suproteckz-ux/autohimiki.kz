...
class KaspiProductionCandidateService
{
    public function list(array $options = []): array
    {
        ...
        foreach ($query->lazyById(100) as $product) {
            if ($force) {
                try {
                    KaspiSingleProductPolicy::assertSku($product->sku);
            ...
            if (! $force && $sku === null && $hasImages && $fileExists && $hasDescription && $hasAttributes && (int) $product->gallery_count > 0) {
                continue;
            }
            if (count($data) === $limit) {
            ...
            $row = ['sku' => $product->sku, 'name' => $product->name,
                ...
                'gallery_count' => $galleryCount, 'has_description' => $hasDescription, 'has_attributes' => $hasAttributes, 'main_image_file_exists' => $fileExists];
            if ($force) {
                $row += KaspiRefreshPolicy::state($product);
            }
            $data[] = $row;
